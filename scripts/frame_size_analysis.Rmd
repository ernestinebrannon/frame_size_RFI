...
```{r task by frame size for aov, include=FALSE, echo=FALSE}
...
#Create dfs for each frame size
...
```{r perception tukey , echo=FALSE}

perception_tukey<- TukeyHSD(p_mag_anova)
print(perception_tukey)

#compact letter display
cld_p<-multcompLetters4(p_mag_anova, perception_tukey)

print(cld_p)
#table with factors and 3rd quantile
tk_p <- group_by(perception, frame_size) %>% 
  summarise(mean=mean(frame_effect_perception), quant = quantile(frame_effect_perception, probs = 0.75)) %>% 
  arrange(desc(mean))

cld_p<- as.data.frame.list(cld_p$frame_size)

tk_p$cld_p<- cld_p$Letters
print(tk_p)

ggplot(perception, aes(frame_size, frame_effect_perception))+
  geom_boxplot(aes(fill=frame_size))+
  labs(x = "Frame Size (\u00b0)", y= "Perceptual Effect(\u00b0)") + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  geom_text(data = tk_p, aes(x = frame_size, y = quant, label = cld_p), size = 3, vjust=-1, hjust =-1)
#filename<- "tukey.png"
#save_plot(filename,tukey_p_plot.png)
...
```{r vv tukey plot, echo=FALSE}


vv_tukey<- TukeyHSD(vv_mag_anova)
print(vv_tukey)

#compact letter display
cld_v<-multcompLetters4(vv_mag_anova, vv_tukey)

print(cld_v)
#table with factors and 3rd quantile
tk_v <- group_by(saccade_to_vert_magnitude, FRAME_SIZE_VAL) %>% 
  summarise(mean=mean(frame_effect_v), quant = quantile(frame_effect_v, probs = 0.75)) %>% 
  arrange(desc(mean))


ggplot(saccade_to_vert_magnitude, aes(FRAME_SIZE_VAL, frame_effect_v))+
  geom_boxplot(aes(fill=FRAME_SIZE_VAL))+
  labs(x = "Frame Size (\u00b0)", y= "Visuovestibular Effect(\u00b0)") + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
```{r, s2v perc corr tests 1, include= FALSE}
#175
...
**Small Frame**
```{r, s2v perc corr tests small}
#175
cor_s2v_perc_175
```

**Medium Frame**
```{r, s2v perc corr tests medium}
#410
cor_s2v_perc_410
```

**Large Frame**
```{r, s2v perc corr tests large}
#645
cor_s2v_perc_645
```

**Extra Large Frame**
```{r, s2v perc corr tests xl}
#880
cor_s2v_perc_880
```
...
```{r small vv model, echo =FALSE}
model_1_small<- lm(vv~perc, df_small)
model_1_small_summary<- summary(model_1_small)
model_1_small_summary
```

**Medium frame**

```{r medium vv model, echo=FALSE}
model_1_medium<- lm(vv~perc, df_medium)
model_1_medium_summary<-summary(model_1_medium)
model_1_medium_summary
```

**Large frame**

```{r large vv model, echo=FALSE}
model_1_large<- lm(vv~perc, df_large)
model_1_large_summary<-summary(model_1_large)
model_1_large_summary
```

**Extra large frame**

```{r xl model vv, echo=FALSE}
model_1_xl<- lm(vv~perc, df_xl)
model_1_xl_summary<- summary(model_1_xl)
model_1_xl_summary
```
...
saccade_to_rod_magnitude$frame_effect_r<-saccade_to_rod_magnitude$frame_effect_r*-1
...
```{r oc tukey , echo=FALSE}

oc_tukey<- TukeyHSD(oc_mag_anova)
print(oc_tukey)

#compact letter display
cld_oc<-multcompLetters4(oc_mag_anova, oc_tukey)

print(cld_oc)
#table with factors and 3rd quantile
tk_oc <- group_by(saccade_to_rod_magnitude, FRAME_SIZE_VAL) %>% 
  summarise(mean=mean(frame_effect_r), quant = quantile(frame_effect_r, probs = 0.75)) %>% 
  arrange(desc(mean))

cld_oc<- as.data.frame.list(cld_oc$FRAME_SIZE_VAL)

tk_oc$cld_oc<- cld_oc$Letters
print(tk_oc)

ggplot(saccade_to_rod_magnitude, aes(FRAME_SIZE_VAL, frame_effect_r))+
  geom_boxplot(aes(fill=FRAME_SIZE_VAL))+
  labs(x = "Frame Size (\u00b0)", y= "Orientation Contrast Effect(\u00b0)") + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  geom_text(data = tk_oc, aes(x = FRAME_SIZE_VAL, y = quant, label = cld_oc), size = 3, vjust=-1, hjust =-1)
#filename<- "tukey.png"
#save_plot(filename,tukey_p_plot.png)
...
**Medium Frame**
```{r medium oc model, echo = FALSE}
oc_medium<- lm(oc~perc, df_medium)
oc_medium_summary<- summary(oc_medium)
oc_medium_summary
``` 

**Large Frame**
```{r large oc model, echo = FALSE}
oc_large<- lm(oc~perc, df_large)
oc_large_summary<- summary(oc_large)
oc_large_summary
```

**Extra Large Frame**
```{r xl oc model, echo=FALSE}
oc_xl<- lm(oc~perc, df_xl)
oc_xl_summary<- summary(oc_xl)
oc_xl_summary
```
...
**Small Frame**
```{r vv oc corr small, echo=FALSE}
df_small %>% 
  group_by(sid,frame_size)

small_vv_oc_cor<-cor.test(df_small$oc, df_small$vv)  
...
ggplot(df_small)+
  ...
  labs(x = "Orientation Contrast Effect", y ="Visuovestibular" )


```
```{r vv oc corr small rep}
report(small_vv_oc_cor)
```
***
**Medium Frame**
```{r vv oc corr medium, echo=FALSE}
df_medium %>% 
  group_by(sid,frame_size)

medium_vv_oc_cor<-cor.test(df_medium$oc, df_medium$vv)  
...
```
```{r vv oc corr medium rep}
report(medium_vv_oc_cor)
```
***
**Large Frame**
```{r vv oc corr large, echo=FALSE}
df_large %>% 
  group_by(sid,frame_size)

large_vv_oc_cor<-cor.test(df_large$oc, df_large$vv)  
...
```
```{r vv oc corr large rep}
report(large_vv_oc_cor)
```
***
**Extra Large Frame**
```{r vv oc corr xl, echo=FALSE}
df_xl %>% 
  group_by(sid,frame_size)

xl_vv_oc_cor<-cor.test(df_xl$oc, df_xl$vv)  
report(medium_vv_oc_cor)  
...
***
**Across Frames**
```{r vv oc corr across, echo=FALSE}
all_task %>% 
  group_by(sid,frame_size)

across_vv_oc_cor<-cor.test(all_task$oc, all_task$vv)  
...
```{r all three task correlation, echo = FALSE}
all_tasks<- all_task %>% 
  mutate(((frame_size=recode(frame_size, "175" = 'Small', "410" = 'Medium', "645" = 'Large', "880" = 'Extra Large'))))

all_task_corrs<-ggplot(all_task)+ 
  geom_point(aes(x=perc,y=vv, color = "blue"))+
  geom_point(aes(x=perc,y=oc, color = "red"))+
  geom_smooth(aes(x=perc,y=oc, color = "red"), method = "lm")+
  geom_smooth(aes(x=perc,y=vv, color = "blue"),method="lm")+
  facet_wrap(~frame_size, nrow=1)+
#  ggtitle("Perception and Combined Saccade Tasks")+
  theme(aspect.ratio = 1)+
  labs(x = "Perception", y = "Saccade Tasks")

all_task_corrs

all_long<-all_task %>% 
  pivot_longer(names_to = "task", cols = c(vv,oc,perc),values_to = "magnitude")

p <- ggplot(data = all_long, aes(x = frame_size, y = magnitude, colour = task, fill = task))+
...
```{r small vv model summary, echo=FALSE}
summary(model_1_small)
```

**Perceptual effect predicted by orientation contrast effect**

```{r small oc model summary, echo=FALSE}
summary(oc_small)
```

**Model comparison**

Model 1: perceptual effect~visuovestibular
Model 2: perceptual ~ visuovestibular+ orientation contrast
...
```{r medium vv model summary,cho = FALSE}
summary(model_1_medium)
```

**Perceptual effect predicted by orientation contrast effect**

```{r medium oc model summary, echo = FALSE}
summary(oc_medium)
``` 
...
```{r large vv model summary, echo=FALSE}
summary(model_1_large)
```

**Perceptual effect predicted by orientation contrast effect**

```{r large oc model summary, echo = FALSE}
summary(oc_large)
```
...
```{r xl model vv summary , echo=FALSE}
summary(model_1_xl)
```

**Perceptual effect predicted by orientation contrast effect**

```{r xl oc model summary, echo=FALSE}
summary(oc_xl)
```
